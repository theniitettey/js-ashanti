...
console.log("[Config] Worker poll interval:", ANALYTICS_CONFIG.WORKER_POLL_INTERVAL_MS, "ms");
console.log("[Config] Recovery interval:", ANALYTICS_CONFIG.RECOVERY_INTERVAL_MS, "ms");
console.log("[Config] Batch seal interval:", ANALYTICS_CONFIG.BATCH_SEAL_INTERVAL_MS, "ms");
...
async function main() {
  try {
    ...
    console.log("  [Worker] Job claiming every", ANALYTICS_CONFIG.WORKER_POLL_INTERVAL_MS / 1000, "seconds");
    console.log("  [Recovery] Stuck job detection every", ANALYTICS_CONFIG.RECOVERY_INTERVAL_MS / 1000, "seconds");
    console.log("  [Batch Sealer] Hybrid sealing (100 events OR 10 min)");
    console.log("  [Job Creator] PENDING jobs for SEALED batches");
    console.log("  [Circuit Breaker] Protects AI API (5 failures → 10 min cooldown)");
  } catch (error) {
